...
impl DataLen for MakeEscrowIxData {
    const LEN: usize = core::mem::size_of::<MakeEscrowIxData>();
}
...
pub fn process_make_escrow(accounts: &[AccountInfo], data: &[u8]) -> ProgramResult {
    ...
    let maker_ata_x_acc = TokenAccount::from_account_info(maker_ata_x)?;
    assert_eq!(maker_ata_x_acc.owner(), maker.key());
